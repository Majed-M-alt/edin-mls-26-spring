...
# Import CuTile decode attention
from decode_attention import decode_attention as cutile_decode_attention

# Global flag to enable/disable FlashAttention
USE_FLASH_ATTENTION = True

# Flag to use CuTile decode attention for seq_q=1
# Disabled: tiled kernel has indexing bug causing NaN
USE_CUTILE_DECODE = False

# Threshold for using fast decode attention vs FlashAttention
DECODE_ATTENTION_THRESHOLD = 4  # Use fast decode if seq_q <= this


def attention_decode_fast(
    q: cp.ndarray,
    k: cp.ndarray,
    v: cp.ndarray,
    scale: float,
    ...
    """
    Fast attention for decode mode (small seq_q).
    Avoids FlashAttention padding overhead for seq_q=1.

    Uses cuBLAS GEMM which is optimized for small matrices.

    Args:
        q: [batch, num_heads, seq_q, head_dim] - contiguous float32
        k: [batch, num_kv_heads, seq_k, head_dim] - contiguous float32
        v: [batch, num_kv_heads, seq_k, head_dim] - contiguous float32
    ...
    # GQA expansion using repeat
    repeats = num_heads // num_kv_heads
    if repeats > 1:
        k = cp.repeat(k, repeats, axis=1)
        v = cp.repeat(v, repeats, axis=1)

    # Q @ K^T using batch matmul
    # q: [batch, heads, seq_q, dim]
    # k: [batch, heads, seq_k, dim] -> k^T: [batch, heads, dim, seq_k]
...
def scaled_dot_product_attention(
    ...
    # V10: Use simple cuBLAS attention for decode mode (seq_q <= DECODE_ATTENTION_THRESHOLD)
    # FlashAttention has issues with small seq_q due to causal mask tile indexing
    if seq_q <= DECODE_ATTENTION_THRESHOLD and attention_mask is None:
        return attention_decode_fast(q, k, v, scale)

    # Try FlashAttention (fast for larger seq_q)
    if USE_FLASH_ATTENTION and attention_mask is None:
        try:
