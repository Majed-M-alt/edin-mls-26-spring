...
def load_safetensor_to_cupy(filepath: str) -> Dict[str, cp.ndarray]:
    ...
    import torch
    tensors = {}
    # Use torch framework to handle bfloat16
...
def load_model_from_hf(model_name: str = "zai-org/GLM-ASR-Nano-2512"):
    ...
    hf_state_np = {k: cp.asnumpy(v) for k, v in hf_state.items()}

    # Load weights
    _load_weights_from_state_dict(cutile_model, hf_state_np)
...
def _load_weights_from_state_dict(model, hf_state):
    """Load weights directly from state dict (numpy arrays).

    Weight key mapping:
    - audio_tower.* -> audio_encoder.*
    - multi_modal_projector.* -> projector.*
    - language_model.* stays the same
    """
    import numpy as np

    def load_linear(layer, weight_np, bias_np=None):
        layer.weight = cp.asarray(weight_np, dtype=cp.float32)
        layer._weight_t_padded = None  # Reset cache
        layer._weight_t_padded_fp16 = None
        if bias_np is not None:
            layer.bias_param = cp.asarray(bias_np, dtype=cp.float32)

    def load_rmsnorm(layer, weight_np):
        layer.weight = cp.asarray(weight_np, dtype=cp.float32)

    def load_layernorm(layer, weight_np, bias_np=None):
        layer.weight = cp.asarray(weight_np, dtype=cp.float32)
        if bias_np is not None:
            layer.bias = cp.asarray(bias_np, dtype=cp.float32)

    def load_embedding(layer, weight_np):
        layer.weight = cp.asarray(weight_np, dtype=cp.float32)

    def load_conv(layer, weight_np, bias_np=None):
        # HF weight is (out_channels, in_channels, kernel_size)
        # CuTile Conv1d expects (out_channels, in_channels * kernel_size)
        out_channels, in_channels, kernel_size = weight_np.shape
        weight_reshaped = weight_np.reshape(out_channels, in_channels * kernel_size)
        layer.weight = cp.asarray(weight_reshaped, dtype=cp.float32)

        # Update padded weight if needed
        if layer.use_cutile and (
            layer.col_size_padded != layer.col_size or
            layer.out_channels_padded != out_channels
        ):
            layer.weight_padded = cp.zeros(
                (layer.out_channels_padded, layer.col_size_padded),
                dtype=cp.float32
            )
            layer.weight_padded[:out_channels, :layer.col_size] = layer.weight
        else:
            layer.weight_padded = layer.weight

        if bias_np is not None:
            layer.bias = cp.asarray(bias_np, dtype=cp.float32)

    # Audio encoder (HF: audio_tower -> CuTile: audio_encoder)
    # Conv layers
    load_conv(model.audio_encoder.conv1,
              hf_state['audio_tower.conv1.weight'],
              hf_state.get('audio_tower.conv1.bias'))
    load_conv(model.audio_encoder.conv2,
              hf_state['audio_tower.conv2.weight'],
    ...
    for i, layer in enumerate(model.audio_encoder.layers):
        ...
        # Self attention (HF: self_attn.* -> CuTile: direct q_proj, etc.)
        load_linear(layer.q_proj,
                    hf_state[f'{prefix}.self_attn.q_proj.weight'],
                    hf_state.get(f'{prefix}.self_attn.q_proj.bias'))
        load_linear(layer.k_proj,
                    hf_state[f'{prefix}.self_attn.k_proj.weight'],
                    hf_state.get(f'{prefix}.self_attn.k_proj.bias'))
        load_linear(layer.v_proj,
                    hf_state[f'{prefix}.self_attn.v_proj.weight'],
                    hf_state.get(f'{prefix}.self_attn.v_proj.bias'))
        load_linear(layer.out_proj,
                    hf_state[f'{prefix}.self_attn.o_proj.weight'],
                    hf_state.get(f'{prefix}.self_attn.o_proj.bias'))

        # MLP
        load_linear(layer.fc1,
                    hf_state[f'{prefix}.mlp.fc1.weight'],
                    hf_state.get(f'{prefix}.mlp.fc1.bias'))
        load_linear(layer.fc2,
                    hf_state[f'{prefix}.mlp.fc2.weight'],
                    hf_state.get(f'{prefix}.mlp.fc2.bias'))

        # Layer norms (HF: input_layernorm -> self_attn_layer_norm, post_attention -> final_layer_norm)
    ...
    # Audio encoder final norm (HF: audio_tower.norm -> CuTile: layer_norm)
    load_layernorm(model.audio_encoder.layer_norm,
                   hf_state['audio_tower.norm.weight'],
                   hf_state.get('audio_tower.norm.bias'))

    # Projector (HF: multi_modal_projector -> CuTile: multi_modal_projector)
    load_linear(model.multi_modal_projector.linear_1,
                hf_state['multi_modal_projector.linear_1.weight'],
                hf_state.get('multi_modal_projector.linear_1.bias'))
    load_linear(model.multi_modal_projector.linear_2,
                hf_state['multi_modal_projector.linear_2.weight'],
                hf_state.get('multi_modal_projector.linear_2.bias'))

    # Text decoder embedding
    load_embedding(model.text_decoder.embed_tokens, hf_state['language_model.model.embed_tokens.weight'])

    # Text decoder layers
    for i, layer in enumerate(model.text_decoder.layers):
        prefix = f'language_model.model.layers.{i}'

        # Self attention (projections are direct layer attributes, not under self_attn)
        ...
        # Reset MLP weight cache
        layer.mlp._gate_weight_t = None
        layer.mlp._up_weight_t = None
        layer.mlp._gate_weight_t_fp16 = None
        layer.mlp._up_weight_t_fp16 = None

        # RMS norms
        load_rmsnorm(layer.input_layernorm, hf_state[f'{prefix}.input_layernorm.weight'])
        load_rmsnorm(layer.post_attention_layernorm, hf_state[f'{prefix}.post_attention_layernorm.weight'])

    # Text decoder final norm
    load_rmsnorm(model.text_decoder.norm, hf_state['language_model.model.norm.weight'])
