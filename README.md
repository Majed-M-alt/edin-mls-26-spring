...
source utils/setup-cutile  # cuTile + hw1
# source utils/setup-triton  # Triton-only (torch, numpy, triton)
...
│   ├── setup-cutile          # cuTile + hw1 environment setup
│   ├── setup-triton          # Triton-only environment setup
...
|-------|------------|------|
| **1. Setup** | Run `source utils/setup-cutile` (or `source utils/setup-triton`) | Get your environment working |
...
These are generated by `conda list --export` and vary across GPU architectures. Use `source utils/setup-cutile` (or `source utils/setup-triton`) for automatic configuration.
